...
function showOptions(game) {
    ...
    for (let i = 0; i < optionButtons.length; i++) {
        ...
        for (let i = 0; i < game.countries.length; i++) {
            name =
                game.countries[
                    Math.floor(Math.random() * game.countries.length)
                ].name;
...
document.addEventListener("DOMContentLoaded", async function () {
    ...
    nextBt.addEventListener("click", () => {
        ...
        let countryName = game.countries[0].name
            .toLowerCase()
            .replace(/\s/g, "");
        ...
        blurryBackground.classList.add("multiple-choice__iconAnswer");
        countryElement.insertAdjacentHTML(
            "beforeend",
            '<div class="multiple-choice__iconAnswer"></div>'
        );
        console.log(blurryBackground);
