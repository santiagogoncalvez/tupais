...
import htmlString from "@components/Flag-gallery/Filters-panel-mobile/Country-search/Options/template.html?raw";

// Styles
import "@components/Flag-gallery/Filters-panel-mobile/Country-search/Options/style.css";

import {
  base,
  modifiers,
} from "@components/Flag-gallery/Filters-panel-mobile/Country-search/Options/Options-class-names.js";
...
export default class Options extends BaseComponent {
  ...
  _addOptionEvents(option, isNormalOption) {
    ...
    option.addEventListener("click", () => {
      ...
      // Cuando se selecciona una opción, el input se limpia
      input.value = "";
      input.focus();
  ...
  selectNextOption() {
    const options = this.dom.querySelectorAll("." + this.base.option);
    let curOpt = this.dom.querySelector("." + this.modifiers.selectedOption.option);
    ...
    if (next) {
      ...
      // Si no hay siguiente → opcional: mantener en último o ciclo
      return null;
    }
  }
  ...
  selectPrevOption() {
    ...
    if (!curOpt) {
      if (options.length > 0) {
        options[options.length - 1].classList.add(this.modifiers.selectedOption.option);
        // Mantener visible
    ...
    if (prev) {
      ...
      // Si no hay anterior → opcional: mantener en primero o ciclo
      return null;
    }
  }
